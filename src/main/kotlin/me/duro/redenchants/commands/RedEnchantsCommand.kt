...
class RedEnchantsCommand : CommandExecutor, TabExecutor {
    private val subcommands = listOf("give", "book", "amount")
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        if (args?.size == 0) {
            sender.sendMessage(replaceColorCodes("&cUsage: /$label <give|book|amount> [args]"))
            return true
        }
        ...
        when (subcommand) {
            ...
            "book" -> {
                ...
                sender.sendMessage(
                    replaceColorCodes(
                        "&7You have received a random ${rarity.color()}${
                            lowerTitleCase(
                                rarity.name
                            )
                        }&7 enchantment book!"
            ...
            "amount" -> {
                val rarities = RedEnchantRarity.entries.minus(RedEnchantRarity.ADMIN)
                val amountPerRarity = mutableMapOf<RedEnchantRarity, Int>()

                rarities.forEach { rarity ->
                    amountPerRarity[rarity] = CustomEnchants.allEnchants.filter { it.enchantRarity == rarity }.size
                }

                amountPerRarity.forEach { (rarity, amount) ->
                    sender.sendMessage(replaceColorCodes("&7${rarity.color()}${lowerTitleCase(rarity.name)}&7: $amount"))
                }
            }
        }
    ...
    override fun onTabComplete(
        ...
        when (subcommand) {
            ...
            "book" -> {
                if (args.size == 2) {
                    return RedEnchantRarity.entries.map { it.name.lowercase() }
